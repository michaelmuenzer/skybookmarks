...
import waitAllFulfilled from '../utils';
import prettifyFilename from '../utils';
...
class ResourceHandler {
	constructor (options, {requestResource, getReference} = {}) {
		this.options = pick(options, supportedOptions);
	...
	getResourceHandler (resource) {
		switch (true) {
			case resource.isCss():
				console.log('using css handler for ' + resource);
				return this.cssHandler;
			case resource.isHtml():
				console.log('using html handler for ' + resource);
				return this.htmlHandler;
			default:
				console.log('using no handler for ' + resource);
				return null;
		}
	}
