...
import { mapSeries, resolve } from 'bluebird';
import utils from '../../utils';
import HtmlSourceElement from './html-source-element';
...
class HtmlResourceHandler {
	...
	handle (resource) {
		...
		return mapSeries(this.allSources, this.loadResourcesForRule.bind(this, $, resource))
			.then(() => {
	...
	loadResourcesForRule ($, parentResource, rule) {
		...
		const promises = $(rule.selector).map((i, element) => {
			...
			if (this.exceedMaxRecursiveDepth(el, parentResource)) {
				console.log(`filtering out ${el} by max recursive depth`);
